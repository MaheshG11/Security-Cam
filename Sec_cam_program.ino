...
else 
{
  ...
  // camera init
  esp_err_t er = esp_camera_init(&config);
  if (er != ESP_OK) 
{
    delay(1000);
    ESP.restart();
...
String capture_send_save(String chat_id,String token) 
{
...
void loop()
{
  pinMode(pir_pin_no, INPUT_PULLUP);
  if (digitalRead(pir_pin_no))
  {
    capture_send_save(ch_id,to_id);
